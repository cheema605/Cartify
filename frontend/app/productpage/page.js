...
export default function ProductPage() {
  ...
  const addToCart = async () => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/shoppping-cart/add-to-cart', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}`,
                      ...
                      <div>
                        {product.is_sellable ? (
                          product.discount_percent && product.discount_percent > 0 ? (
                            <>
                              <span className="text-3xl font-bold text-gray-900 line-through">Rs {product.price.toFixed(2)}</span>
                              <span className="text-3xl font-bold text-red-600 ml-2">Rs {product.discounted_price.toFixed(2)}</span>
                              <span className="text-sm text-red-500 ml-2">({product.discount_percent}% off)</span>
                            </>
                          ) : (
                ...
                <Card>
                  <CardContent className="pt-4 md:pt-6 px-4 md:px-6">
                    <div className="flex flex-col gap-4">
                      <div>
                        {product.is_rentable ? (
                          <span className="text-3xl font-bold text-gray-900">Rs {product.rent ? product.rent.toFixed(2) : (product.price * 0.1).toFixed(2)}</span>
                        ) : (
                          <span className="text-3xl font-bold text-gray-900">N/A</span>
                        )}
                        <div className="flex items-center space-x-1 mt-2">
